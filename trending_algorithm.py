...
def test_v5_algorithm():
    ...
    test_videos = [
        VideoData(
            video_id="test1",
            title="Bundesliga Highlights: Bayern München vs Dortmund 2025",
        ...
        VideoData(
            video_id="test2", 
            title="Duggu ki mummy kyu rone lagi💔😭||Puja Dhamija shorts viral",
            channel="Puja Robin Actor",
            views=2000000,
            comments=100000,  # 5% Comment-Rate = sehr verdächtig
            likes=80000,
            duration_seconds=58,
            age_hours=8,
        ...
        VideoData(
            video_id="test4",
            title="Tech Review: iPhone 16 Pro Max erste Eindrücke",
    ...
    print("🧪 V5.0 ENHANCED ALGORITHM TEST")
    print("=" * 60)
    
    # Teste V5.0 Regional Algorithm für Deutschland
    ...
    print(f"\n📊 V5.0 ANALYSIS RESULTS (Target: DE):")
    print("=" * 60)
    
    for result in results:
        flags = []
        if result.is_indian_content: flags.append("🚫 INDIAN")
        if result.is_regional_content: flags.append("🇩🇪 GERMAN")
    ...
    print("📈 V5.0 FILTER STATISTICS:")
    print("=" * 60)
    for key, value in filter_stats.items():
        print(f"   {key}: {value}")
    
    print(f"\n🎯 V5.0 SUCCESS METRICS:")
    print(f"   ✅ Indian videos limited: {filter_stats['indian_videos_kept']}/{filter_stats['indian_videos_found']}")
    print(f"   ✅ German videos boosted: {filter_stats['german_videos_boosted']}")
    print(f"   ✅ Quality filter applied: {len(results)} results")
